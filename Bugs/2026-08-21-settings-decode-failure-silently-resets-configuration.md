...
自动化覆盖解码失败的判定、备份、上报与提示文案组装。**未验证**：真实用户在真实损坏场景下的完整体验、提示在当前生产 `minSize`（以 `RemoteMicApp.swift` 为准，现为 `1020 × 772`）与更大窗口下的布局表现（更窄布局用 `REMOTE_MIC_SETTINGS_SCREENSHOT_SIZE` 离屏渲染，注明为离屏结果）、以及中英文实际渲染字号。
