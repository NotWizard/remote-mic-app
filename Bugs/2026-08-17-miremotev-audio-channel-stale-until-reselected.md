...
代码中真实存在“设备仍显示已检测到，但实际音频通道健康状态没有被完整验证”的检测缺口。用户描述的问题具有可信的软件原因，手动重新选择能够恢复也符合当前重建音频引擎的实现。

由于没有现场日志和真机复现，当前不能把 H1、H2、H3 或 H4 中任何一项写成已确认根因，也不能确认问题只发生在某个版本、某种遥控器或某个第三方 App。

## 候选优化方案

以下仅为后续修复方案，本次没有实施：

1. 在蓝牙语音开始、手机/网页语音开始、蓝牙重新 ready、Mac 唤醒和音频配置变化时，统一调用更完整的音频健康检查，而不是只读取 `isAudioOutputReady`。
2. 健康检查至少覆盖：引擎运行、播放器播放状态、实际绑定设备 UID/ID、设备是否仍可用，以及最近入队缓冲是否持续完成播放。
3. 检测到异常时执行一次去抖、限次的自动重绑；语音已经开始时短暂缓存首批音频，避免重绑期间丢失开头。
4. Mac 唤醒后等待 CoreAudio 稳定，再进行一次音频检查；必要时增加一次延迟复查，不使用持续定时强制重启。
5. 将界面状态拆分为“驱动已安装”“音频通道已连接”“正在恢复”和“音频不可用”，避免把“已检测到”误解为整条链路正常。
6. 已选择该设备时，将手动操作改为含义明确的“重新连接 MiRemoteV 2ch”，作为自动恢复失败时的兜底。
7. 如果只发现单个第三方 App 输入流失效，提示用户在该 App 内重新选择麦克风；macOS 没有通用接口强制所有第三方 App 重开输入流。

## 计划验证

正式修复前必须先取得可重复触发条件和对应时间段日志。修复后至少覆盖：
...
- 本轮没有反馈机器的现场日志，没有执行真机复现，也没有运行修复验证。
- 静态代码检查可以确认状态检测缺口和手动按钮的重绑行为，不能确认失效发生时 CoreAudio、虚拟驱动和第三方 App 的真实状态。
- 单元测试和模拟只能验证恢复决策；睡眠唤醒、真实 CoreAudio 驱动、蓝牙时序和第三方 App 最终收音必须使用真实 Mac、真实遥控器和真实语音工具验收。
